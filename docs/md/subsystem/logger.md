...
## 日志输出控制
在图形化配置中的`日志`菜单下开发者可以启用或禁用日志的输出，分别为：
- 调试日志（DBG）
- 信息日志（INFO）
- 错误日志（ERROR）
- FPS日志输出

启用调试日志后可以在`调试日志选项`中对调试日志进行更深的控制，分别为：
- 布局与测量日志
- 渲染日志（每帧的 `render start` / `render done` 等粗粒度日志）
- 渲染详细日志（逐控件/逐块的绘制日志，需先启用渲染日志）
- 事件日志

对于未启用的日志其输出日志的宏会被替换为空语句。
