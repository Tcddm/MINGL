...
有关更多图形缓冲的信息请访问[图形缓冲](/core/framebuffer)
:::
...
### alpha混合
::: tip
如果你有硬件加速实现请将**MGL_HW_ACCEL_ALPHA_BLEND**设置为**1**并且实现以下所有函数

图形化配置路径为`硬件加速特性→alpha混合`
:::
::: tip
默认实现为alpha<128不绘制，否则直接绘制
:::
<<< @/../../src/hal/mgl_hal.h#mgl_hal_fill_rect_alpha{c}

<<< @/../../src/hal/mgl_hal.h#mgl_hal_bit_blt_alpha{c}

以下函数为便捷函数：

<<< @/../../src/hal/mgl_hal.h#mgl_hal_set_pixel_alpha{c}
