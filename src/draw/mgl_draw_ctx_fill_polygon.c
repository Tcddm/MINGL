    ...
    for(mgl_coord_t y=y_min;y<=y_max;y++){
        ...
        for(uint16_t i=0;i<cnt;i++){
            ...
            if((y1>y)!=(y2>y)){
                ...
                int32_t temp=(int32_t)x1*dy+dx*((int32_t)y-y1);
                int32_t x=temp/dy;
                int32_t rem=temp%dy;
                if((rem!= 0 && dy>0) || (rem!=0 && dy<0 && temp<0)){
                    x+=1;
                }
                //数组越界保护
        ...
        for(mgl_coord_t i=0;i<n-1;i+=2){
            if(xi[i]>=xi[i+1]){ continue;}
            if(xi[i]<xi[i+1]){
                mgl_ctx_draw_line(ctx,xi[i],y,(mgl_coord_t)(xi[i+1]-1),y,1,painter);
            }
        }
    }
}